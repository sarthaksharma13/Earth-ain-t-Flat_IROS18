...
public:
	...
	int getNumCars() const { return numCars_; }
	...
	bool loadFile(const char *fileName){
		...
		//numCars
		fscanfOrDie(fptr, "%d", &numCars_);
		// numPts
		fscanfOrDie(fptr, "%d", &numPts_);

		// Center of the car
		carCenter_ = new double[numCars_*3];
		for(int c=0;c<numCars_;c++){
			fscanfOrDie(fptr, "%lf", carCenter_ + 3*c + 0);
			fscanfOrDie(fptr, "%lf", carCenter_ + 3*c + 1);
			fscanfOrDie(fptr, "%lf", carCenter_ + 3*c + 2);
		...
		// Observations
		observations_ = new double[numCars_*2*numPts_];
		for( int c=0;c<numCars_;c++)
			for(int i = 0; i < numPts_; ++i){
				for(int j = 0; j < 2; ++j){
					fscanfOrDie(fptr, "%lf", observations_ + c*2*numPts_ + i*2 + j);
			}
		}
		/*for( int c=0;c<numCars_;c++)
			for(int i = 0; i < numPts_; ++i){
				for(int j = 0; j < 2; ++j){
					std::cout << observations_[c*2*numPts_ + i*2 + j] << std::endl;
			}
		}*/

		// Observation weights
		observationWeights_ = new double[numCars_*numPts_];
		for(int c=0;c<numCars_;c++)
			for(int i = 0; i < numPts_; ++i){
				fscanfOrDie(fptr, "%lf", observationWeights_ + c*numPts_ + i);
		}

		// Mean locations
		X_bar_ = new double[numCars_*3*numPts_];
		for(int c=0;c<numCars_;c++)
			for(int i = 0; i < numPts_; ++i){
				for(int j = 0; j < 3; ++j){
					fscanfOrDie(fptr, "%lf", X_bar_ + c*3*numPts_ + i*3 + j);
	...
	/*	for(int c=0;c<numCars_;c++)
			for(int i = 0; i < numPts_; ++i){
				for(int j = 0; j < 3; ++j){
					std::cout << X_bar_[c*3*numPts_ + i*3 + j] << std::endl;
		...
		/*std::cout<<numVec_<<std::endl;*/

		// Read in the top 42 eigenvectors for the shape
		// Size allocation: 42 vecs * 3 coordinates per vex * 36 keypoints (numPts_)
		V_ = new double[numCars_*numVec_*3*numPts_];
		for(int c=0;c<numCars_;c++)
			for(int i = 0; i < numVec_; ++i){
				for(int j = 0; j < numPts_; ++j){
					fscanfOrDie(fptr, "%lf", V_ + c*numVec_*3*numPts_+ i*3*numPts_ + 3*j + 0);
					fscanfOrDie(fptr, "%lf", V_ + c*numVec_*3*numPts_+ i*3*numPts_ + 3*j + 1);
					fscanfOrDie(fptr, "%lf", V_ + c*numVec_*3*numPts_+ i*3*numPts_ + 3*j + 2);
				}
			}
		...
		/*for(int i = 0; i < numVec_; ++i){
			std::cout<<lambdas_[i] << std::endl;
		}
*/
		

		// Read the plane parameters
		plane_ = new double[4];
		for(int i=0;i<4;i++){
			fscanfOrDie(fptr, "%lf", plane_ + i);	
		}
		/*for(int i=0;i<4;i++){
			std::cout << plane_[i] << std::endl;
		}*/
